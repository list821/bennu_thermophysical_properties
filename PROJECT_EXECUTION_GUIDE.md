...
改正重复除以 `FILL_FAC` 后，当前公开输入下的审计扫描最优值为 Γ=184 J m⁻² K⁻¹ s⁻½，但约化 χ²=433.14，统计上不可接受。因此它是“当前公开数据和孔径模型下的数值最小值”，不是对论文 Γ=350±20 的独立替代测量。
...
- `thermal_radiance`：与探测器量相同，直接用于和 CK/IK 孔径积分模型比较；
- `disk_equivalent_radiance_diagnostic = detector/FILL_FAC`：只作诊断展示，不参加拟合。
...
& 'C:\Users\松廷\.cache\codex-runtimes\codex-primary-runtime\dependencies\python\python.exe' `
  ...
  --gamma-min 178 --gamma-max 192 --gamma-step 1 `
  --model-phases 48 `
...
最优 Γ=184，但与论文 350±20 有显著差异，原因按重要性分为：

1. **作者 L3a 未公开**：公开档案是 L2 v2。校准文档明确提示目标欠填充 FOV 会产生光谱伪影，但作者使用的逐帧/逐波长经验校正数组没有公开。`FILL_FAC` 现在只保留为几何诊断量，不再参与观测辐亮度换算。
2. **观测/模型归一化曾混用，现已修正**：探测器视场平均约 1.12×10⁻⁴；旧代码除以 0.366 后错误地用约 3.07×10⁻⁴ 拟合盘面模型。现在观测保持探测器口径，并与 `mixed_detector()` 孔径积分模型比较。
...
7. **误差模型不完备**：1° 箱误差包含统计散布、传播误差和 0.5% 下限，却没有作者的系统协方差。约化 χ²=433.14 表明残差由系统误差主导，Δχ²=1 不能作为可信物理误差条。
